...
public class BankSystem{
	...
	public static void createCustomerAccount(String customerid, String firstName, String username, String lastName, int age, String address,long mobileNumber, String eMailId, String dateofbirth, int balance) {
		try {
			...
			if(rows>0) {
				System.out.println("Given information inserted successfully.....");
			}
		}
	...
	public static void depositMoney(String customerid,String username,int amt) {
		try {
		
		PreparedStatement ps = BankDb.conn.prepareStatement("select balance from customer where customerid = ?");
		...
		int rows = stmt.executeUpdate();
		if(rows > 0) {
			System.out.println("Money deposited successfully....The total balance is"+(amt+rs.getInt(1)));
		}
		}
		catch(SQLException e) {
			e.printStackTrace();
